...
class Main_model extends CI_Model {
    ...
    public function userInfoByUserId($user_id){
        $this->db->select('*');
    ...
    public function get_servs($type) {
        ...
        $this->db->select('service_id,bill_fetch,billing_model,code,description,is_bbps,is_down,max_amt,max_len,min_amt,min_len,service_name,type,verify_outlet,validate_required');
    ...
    function getnoteDataWithRole($roleid='') {
        $result = array();
        $notif_for = array();
        ...
        $this->db->where_in('notif_for', $notif_for);
        $this->db->where('valid_upto >=', date('Y-m-d H:i:s'));
 ...
 public function check_exstng_doc($acntid, $doc) {
    ...
    public function AddPlanData($PlanName, $PlanCode, $Description, $PlanFor, $IsActive, $uid) {
      ...
      public function PlanServiceListData($plan) {
        $this->db->select('psc.*,s.service_name,s.code,s.type as servicetype,pcg.plan_name');
        $this->db->from('plan_service_config psc');
        $this->db->join('plan_config pcg','pcg.plan_id=psc.plan_id');
    ...
    public function plan_name_by_planid($plnid){
        ...
        foreach ($Service as $key => $val) {
            ...
            if($val['CappingAmount'] && $val['CappingAmount']>0){
                $data['capping_amount'] = $val['CappingAmount'];
            }
       ...
       // $this->db->select('psc.pl_srvc_rl_id as PlanServiceConfig,psc.charge_type,psc.charge_method,s.service_name,s.code');
        $this->db->select('psc.service_id,s.service_name,s.code,s.type as servicetype');
        $this->db->from('plan_service_config psc');
        $this->db->join('services s', 'psc.service_id = s.service_id');
    ...
    public function PlanSlabApplicableList($serviceId,$planid) {
        $this->db->select('psrvc.*,pcg.plan_name');
        ...
        $this->db->join('plan_config pcg','pcg.plan_id=psc.plan_id');
        $this->db->where(array('psrvc.service_id' =>$serviceId,'psc.plan_id'=>$planid));
        $sel = $this->db->get();
        $q = $sel->result_array();
        if ($q) {
            return $q;
    ...
    public function checkServiceConfigExist_in_plan($plan_id,$serviceid) {
        $this->db->from('plan_service_config');
        $this->db->where(array('plan_id' => $plan_id,'service_id'=>$serviceid));
        $sel = $this->db->get();
        $q = $sel->row_array();
        if ($q) {
            return $q;
    ...
    public function check_load_amt_userid($userid) {
        $this->db->select_sum('amount');
        $this->db->where(array('txn_type'=>'DEPOSIT','user_id' => $userid, 'date_format(`updated_on`,"%Y-%m-%d")'=>date('Y-m-d')));
        $sel = $this->db->get('credit_history'); 
        $q = $sel->row_array();
        // echo $this->db->last_query();die;
        if ($q) {
            return $q;
    ...
    public function servc_txn_params($service_id,$all=false) {
        
        if($all===true)
        {
            $this->db->order_by('apb_order', 'ASC');
            $this->db->select('*');
        }else{
            $this->db->order_by('order', 'ASC');
            $this->db->select('param_name,param_type,param_code,order,full_regex,start_regex,general_regex');
        }
    ...
    function InsertEvent($userfile, $validup) {
        $validup = date('Y-m-d H:i:s', (strtotime(date_format(date_create($validup), "Y-m-d")) + 86399));
        // echo $validup;die;
        return $this->db->insert('portal_notifs', array('heading' => 'GREETING', 'content' => $userfile, 'notif_for' => 0, 'valid_upto' => $validup, 'created_on' => date('Y-m-d H:i:s')));
    }


    public function InsertFeedbackPost($insert){
        return $this->db->insert('feedback_post',$insert);
    }
    ...
    public function getFeedbackPost(){
        $result = array();
        $this->db->select('fbp.*, u.first_name,u.last_name,u.mobile,u.email, r.role_name');
        $this->db->from('feedback_post fbp');
        $this->db->join('users u', 'u.user_id = fbp.user_id');
    ...
    public function check_if_loginip_exist($user_id,$ip)
    {
        $this->db->from('login_ip_dump');
        $this->db->where(array('user_id'=>$user_id,'ip'=>$ip));
        return $this->db->count_all_results();
    }

    
    public function log_login_ip_foruser($user_id)
    {
        $ip=ip_address();
        $insert_array=array(
        "user_id"=>$user_id,
        ...
        $this->db->insert('login_ip_dump',$insert_array);
    }

    public function get_pagedata($pageid)
    {
        $sel=$this->db->get_where('navbar',array('id'=>$pageid,'is_active'=>1));
        $f=$sel->row_array();
        if($f)
        {
            return $f;
    ...
    public function check_txntable_txnid($txnid) 
    {
        if ($txnid) {
            $s = $this->db->get_where('usertxn_table', array('fstpytxn_id' => $txnid));
            $f = $s->row_array();
            if ($f) {
                return $f;
            }
    ...
    public function get_vendor_details_for_outlet_request()
    {
        $sel=$this->db->get_where('vendor_list',array('vendor_id'=>1));
        $f=$sel->row_array();
        if($f)
        {
            return $f;
    ...
    public function get_distinct_pincodedt($pincode)
    {
        
        $sel=$this->db->get_where('distinct_pincode',array('pincode'=>$pincode));
        $f=$sel->row_array();
        if($f)
        {
            return $f;
    ...
    public function get_retailer_wise_servicebreakup($user_id,$result_type)
    {
     
        $valid=true;
        
        $this->db->select('COUNT(ut.id) as total_count_txn,SUM(ut.transamt) as total_sum_txn,COALESCE(s.category_type,\'NA\') as category_type');
        $this->db->from('usertxn_table ut');
        $this->db->join('services s','ut.scode=s.code');
        
        if($result_type=="Today")
        {
            
            $this->db->where('DATE_FORMAT(ut.req_dt,\'%Y-%m-%d\')',date('Y-m-d'));
            
        }else if($result_type=="Samemonth")
        {
            
            $this->db->where('DATE_FORMAT(ut.req_dt,\'%Y-%m\')', date('Y-m'));
            
        }else if($result_type=="Previousmonth")
        {
            $this->db->where('DATE_FORMAT(ut.req_dt,\'%Y-%m\')', date('Y-m', strtotime(date('Y-m')." -1 month")));
        }else{
            $valid=false;
        }
        ...
        if($valid===true)
        {
            $this->db->where(array('ut.user_id'=>$user_id,'ut.status!='=>'REFUND'));
            $this->db->group_by('s.category_type');
            $sel=$this->db->get();
            $f=$sel->result_array();
            if($f)
            {
    ...
    public function get_distinct_service_categories()
    {
        $this->db->order_by('category_type','ASC');
    ...
    public function get_allchild_userid($user_id)
    {
        $sel=$this->db->get_where('users',array('parent_id'=>$user_id));
        $f=$sel->result_array();
        $data_dt=array();
        if($f)
        {
            ...
            foreach($f as $uk=>$uv)
            {
                $getchilddata=$this->get_allchild_userid($uv['user_id']);
                if($getchilddata)
                {
                    $data_dt=array_merge($data_dt,$getchilddata);
                }
            }
    ...
    public function get_childs_wise_servicebreakup(array $child_array,$result_type)
    {
        if(count($child_array)>0)
        {
        $valid=true;
        
        $this->db->select('GROUP_CONCAT(DISTINCT CONCAT(\'SUM(IF(s.category_type="\',s.category_type,\'",transamt,0)) AS \',s.category_type)) as head');
        $this->db->from('usertxn_table ut');
        $this->db->join('services s','ut.scode=s.code');
        $sel=$this->db->get();
        $f=$sel->row_array();
        if($f)
        {
            ...
            $this->db->select('ut.user_id,CONCAT(MIN(COALESCE(u.first_name,\'\'))," ",MIN(COALESCE(u.last_name,\'\'))) as name,'.$f['head']);
            $this->db->from('usertxn_table ut');
            $this->db->join('services s','ut.scode=s.code');
            $this->db->join('users u','u.user_id=ut.user_id','left');
            if($result_type=="Today")
            {
            ...
            if($valid===true)
            {
                $this->db->where(array('ut.status!='=>'REFUND'));
                $this->db->where_in('ut.user_id',$child_array);
                $this->db->group_by('ut.user_id');
                $sel=$this->db->get();
                $f=$sel->result_array();
                if($f)
                {
     ...
     public function get_allusers_servicebreakup($result_type)
    {
    ...
    public function configure_aeps_for_userifnot_exist($user_id)
    {
        ...
        if($f)
        {
            ...
            "created_useragent"=>$this->agent->agent_string()
            );
            
            $insert_settlement_dt=$this->db->insert('user_settlement_config',$insert_array);
            
            if($insert_settlement_dt)
            {
                return true;
            }
    ...
    public function find_last_userfor_role($roleid)
    {
        $this->db->limit(1);
        $this->db->order_by('id','DESC');
        $sel=$this->db->get('users',array('role_id'=>$roleid));
        $f=$sel->row_array();
        if($f)
        {
            return $f;
    ...
    public function get_all_services_inplan($planid)
    {
        $this->db->order_by('srv.type','ASC');
        $this->db->select('srv.service_name,srv.billing_model,srv.type,srv.min_amt,srv.max_amt,(CASE WHEN srv.is_down=1 THEN \'UP\' ELSE \'DOWN\' END) as service_status,psc.charge_type,psc.charge_method,psc.rate,psc.capping_amount,psc.slab_applicable,psc.plan_id,psc.service_id');
        $this->db->from('plan_config pc');
        $this->db->join('plan_service_config psc','pc.plan_id=psc.plan_id');
        $this->db->join('services srv','srv.service_id=psc.service_id');
        $this->db->where(array('pc.is_active'=>1,'pc.plan_id'=>$planid,'srv.is_active'=>1));
        $sel=$this->db->get();
        $f=$sel->result_array();
        if($f)
        {

                                    ...
                                    if ($chkservicestat['gateway_down'] == 0) {
                                        if ($chkservicestat['type'] == 'PREPAID') {
                                             if ($chkservicestat['Amnt_by_routing'] == 1) {
                                                ...
                                                $result = $this->{$chck_amnt_rng['vendor_library']}->prepaid_rech($user_info, $user_info['plan_id'], $param['account'], $param['amount'], $chkservicestat);
 ...
 */
    ...
    $curl = curl_init();
	  ...
	  CURLOPT_URL => "https://www.instantpay.in/ws/api/serviceproviders?token=19b6aaa0623d1a724d9ba7691652dbb8&spkey=VFP&format=json",
...
curl_setopt_array($curl, array(
  CURLOPT_URL => "https://www.instantpay.in/ws/api/transaction?format=json&token=19b6aaa0623d1a724d9ba7691652dbb8&spkey=VFP&agentid=Z19109319992JMTU&account=8851245444&amount=12",
...
// FASTAG   PBT  BFT AFT ICT  BBT     BROADBAND  VFB TTB TIB TBB SBB NBB NPB MNB INB ONB//
    public function bbps_biller(){

        $request['token'] = '19b6aaa0623d1a724d9ba7691652dbb8';
        $request['request']['sp_key'] = 'PBT';
		$request['request']['service_type']='FASTAG';
        $curl = curl_init();
        curl_setopt_array($curl, array(
          CURLOPT_URL => "https://www.instantpay.in/ws/userresources/bbps_biller",
          CURLOPT_RETURNTRANSFER => true,
          CURLOPT_ENCODING => "",
          CURLOPT_MAXREDIRS => 10,
          CURLOPT_TIMEOUT => 0,
    ...
    public function bill_fetch(){
        $request['token'] = '19b6aaa0623d1a724d9ba7691652dbb8';
        $request['request']['sp_key'] = 'DDW';
        $request['request']['agentid']='1234567899';
        $request['request']['customer_mobile']='9999999999';
        ...
        $request['request']['outletid']='250';

        $curl = curl_init();
        curl_setopt_array($curl, array(
          CURLOPT_URL => "https://www.instantpay.in/ws/bbps/bill_fetch",
          CURLOPT_RETURNTRANSFER => true,
          CURLOPT_ENCODING => "",
          CURLOPT_MAXREDIRS => 10,
          CURLOPT_TIMEOUT => 0,
      ...
      public function bill_pay(){
        ...
        $request['request']['outletid']='30598';
    ...
    public function GetKYCDocument (){
        $request['token'] = '19b6aaa0623d1a724d9ba7691652dbb8';
        $request['request']['outletid'] = '30598';
        $request['request']['pan_no']='CFAPP9240L';
      

        $curl = curl_init();
        curl_setopt_array($curl, array(
          CURLOPT_URL => "https://www.instantpay.in/ws/outlet/requiredDocs",
          CURLOPT_RETURNTRANSFER => true,
          CURLOPT_ENCODING => "",
          CURLOPT_MAXREDIRS => 10,
          CURLOPT_TIMEOUT => 0,

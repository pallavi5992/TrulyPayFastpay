...
//                                    if($get_agentcode){
                                        ...
                                        $get_agentcode['agent_code']='30598';
                                    ...
                                    $get_agentcode['agent_code']='30598';
                                        ...
                                        if (file_exists(APPPATH . "libraries/" . ucfirst($chkservicestat['vendor_library']) . ".php")) {
                                            ...
                                            if (ctype_digit($param['TXN_REQUEST']['transamount'])) {
                                                ...
                                                $data['outletname']=trim($user_info['business_name']);
                                            } else {
                                                $data['error'] = 1;
                                                $data['error_desc'] = "Invalid Transaction Amount";
                                                $data['msg'] = null;
...
//									}
    ...
    public function get_benevalidation_commercials()
    {
            if ( $_SERVER['REQUEST_METHOD'] === 'POST') 
            {
                $e = $this->session->userdata('userid');
                ...
                if ($user_info) 
                {
                    if ($user_info['role_id']==4) 
                    {
                        ...
                        {
                        	$this->load->model('Inst_model');        
                            $chkservicestat = $this->Inst_model->fetch_service_prov('DMV');
                            if ($chkservicestat) 
                            {
                                ...
                                {
                                    $this->load->library($chkservicestat['vendor_library']);
                                    $result = $this->{$chkservicestat['vendor_library']}->fetch_bene_validationcommercials($user_info, $chkservicestat);
    ...
    public function VerifyBeneficiaryAccount()
    {
                ...
                if ($user_info) 
                {
                    if ($user_info['role_id']==4) 
                    {
                        if ($user_info['is_block'] == 0) 
                        {
                            ...
                            $this->load->library($chkservicestat['vendor_library']);
                            $result = $this->{$chkservicestat['vendor_library']}->VerifyBeneficiaryAccount($user_info, $param,$chkservicestat);
           ...
           $user_info = $this->Main_model->user_acntid($e);
            if ($user_info) {
             ...
             <table border="0" cellpadding="0" cellspacing="0" width="100%">
                    ...
                    foreach ($rowdata as $k=>$c) 
                    {
					
                        $get_txndetails=$this->Main_model->check_txntable_txnid($c['trans']);
                        
                        if($get_txndetails)
                        {
                            
                            if($get_txndetails['user_id']==$user_info['user_id'] && $get_txndetails['servicetype']=='REMITTANCE')
                            {
                                
                        
                        $conten = $conten . '<tr>
                                             <td align="center" valign="top" style="padding: 5px;border:1px solid #ddd;">' . $get_txndetails['fstpytxn_id'] . '</td>
                                             <td align="center" valign="top" style="padding: 5px;border:1px solid #ddd;">' . $get_txndetails['op1'] . '</td>
                                             <td align="center" valign="top" style="padding: 5px;border:1px solid #ddd;">' . $get_txndetails['customer_no'] . '</td>
                                             <td align="center" valign="top" style="padding: 5px;border:1px solid #ddd;">' . $get_txndetails['op2'] . '</td>
								             <td align="center" valign="top" style="padding: 5px;border:1px solid #ddd;">' . $get_txndetails['op3'] . '</td>
                                             <td align="center" valign="top" style="padding: 5px;border:1px solid #ddd;">' . $get_txndetails['op4'] . '</td>
											 <td align="center" valign="top" style="padding: 5px;border:1px solid #ddd;">' . $get_txndetails['op6'] . '</td>
											 <td align="center" valign="top" style="padding: 5px;border:1px solid #ddd;">' . $get_txndetails['transamt'] . '</td>
                                             <td align="center" valign="top" style="padding: 5px;border:1px solid #ddd;">' . $get_txndetails['opr_ref_no'] . '</td>
                                             <td align="center" valign="top" style="padding: 5px;border:1px solid #ddd;">' . $get_txndetails['req_dt'] . '</td>
                                             <td align="center" valign="top" style="padding: 5px;border:1px solid #ddd;">' . $get_txndetails['status'] . '</td>
                                             </tr>';
                            }
                   ...
                   $this->m_pdf->pdf->WriteHTML($conten);
                    ...
                    if (count($rowdata) == 1) 
                    {
                        ...
                        $name = $rowdata[0]['trans'].".pdf";

                    } else {

                        $name = "MoneyTransfer Receipts.pdf";

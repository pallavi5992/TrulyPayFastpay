        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($user_info) {    
                                        ...
                                        if ($Mobile && strlen($Mobile) == 10) {
                                            ...
                                            // $pswd = $letters[mt_rand(0, 51)] . mt_rand(100, 999) . $letters[mt_rand(0, 51)] . $letters[mt_rand(0, 51)] . $letters[mt_rand(0, 51)] . mt_rand(0, 9);   
                                             ...
                                             $pswd=mt_rand(100000,999999);
                                                    ...
                                                    $accnt_rand = account_id_creation($Role);   
                ...
                $tk = $this->service_alerts->UAC('UAC',$Mobile, $Email, $BusinessName,$checkRoleValid['role_name'], $accnt_rand, $pswd, 'User Creation');
                                            ...
                                            $data['error'] = 1;
                                            $data['error_desc'] = 'Unable to create user';
                                            $data['msg'] = NULL;
                                            }                      
                            ...
                            if (ctype_digit($param)) {
                                ...
                                if ($vendr) {
                                    if ($vendr['bal_check_api'] == 1) {
                                        ...
                                        if (file_exists(APPPATH . "libraries/" . ucfirst($vendr['vendor_library']) . ".php")) 
                                        {
                                        
                                        $this->load->library($vendr['vendor_library']);
                                            
                                        if (method_exists($this->{$vendr['vendor_library']}, 'ApiBalanceCheck'))
                                        {     
                                        ...
                                        if ($data['error'] == 0) {
                                            ...
                                            $data['error'] = 1;
                                            $data['error_desc'] = 'Unable to find balance check function in vendor library';
                                            $data['msg'] = null;
                                        }
                                            
                                        }else{
                                            $data['error'] = 1;
                                            $data['error_desc'] = 'Unable to find vendor library';
                                            $data['msg'] = null;
                                        }
                       ...
                       if(page_access(22))
                        {
                            ...
                            if($PlanServiceListData)
                            {
                                foreach($PlanServiceListData as $pk=>$pv)
                                {
                                    $PlanServiceListData[$pk]['slabdata']=array();
                                    if($pv['slab_applicable']==1)
                                    {
                                        $slabdata=$this->Main_model->PlanSlabApplicableList($pv['service_id'],$pv['plan_id']);
                                        $PlanServiceListData[$pk]['slabdata']=$slabdata?$slabdata:array();
                                        
                                    }
                                    
                                    unset($PlanServiceListData[$pk]['pl_srvc_rl_id']);
                                   // unset($PlanServiceListData[$pk]['service_id']);
                                   // unset($PlanServiceListData[$pk]['plan_id']);
            ...
            if ($user_info) {
                    if ($user_info['is_block'] == 0) {
                        ...
                        if(page_access(22))
                        {
                            ...
                            $ServiceConfig = isset($params['service_id'])?(is_string($params['service_id'])?trim($params['service_id']):""):"";
                            $plan_id= isset($params['plan_id'])?(is_string($params['plan_id'])?trim($params['plan_id']):""):"";
                            
                            if($ServiceConfig!="" && $plan_id!="")
                            {
                                
                                
                                
                                $PlanSlabApplicableList = $this->Main_model->PlanSlabApplicableList($ServiceConfig,$plan_id);
            ...
            if ($user_info) {
                    ...
                    if ($user_info['is_block'] == 0) {
                        ...
                        if(page_access(22))
                        {
                            ...
                            $ServiceConfigId = $this->input->post('service_id');
                            $SlabRates = $this->input->post('SlabRates');
                            $plan_id=$this->input->post('plan_id');
                            $ServiceConfigId = is_string($ServiceConfigId)?trim($ServiceConfigId):"";
                            $plan_id=is_string($plan_id)?trim($plan_id):"";
                        
                        $checkServiceConfigExist = $this->Main_model->checkServiceConfigExist_in_plan($plan_id,$ServiceConfigId);
                        if ($checkServiceConfigExist) {     
                         if(is_array($SlabRates) && count($SlabRates)>0){  
                            ...
                            foreach ($SlabRates as $key => $value) {
                                ...
                                if(!in_array($value['ChargeType'],array_keys($b['ChargeType'])))
                                {
                                        $data['error'] = 1;
                                        $data['error_desc'] = 'Invalid Charge Type';
                                        $data['msg'] = NULL;
                                        echo json_encode($data);
                                        exit;
                                }
                                
                                if(!in_array($value['ChargeMethod'],array_keys($b['ChargeMethod'])))
                                {
                                        $data['error'] = 1;
                                        $data['error_desc'] = 'Invalid Charge Method';
                                        $data['msg'] = NULL;
                                        echo json_encode($data);
                                        exit;
                                }
                            ...
                            foreach ($SlabRates as $k => $v) {
                                ...
                                $insertSlabData[$k] = array(
                                    'plan_id'=>$checkServiceConfigExist['plan_id'],
                                    'service_id'=>$checkServiceConfigExist['service_id'],
                                    'min_amnt' => $v['SlabMinimumAmount'],
                                    'max_amnt' => $v['SlabMaximunAmount'],
                                    'rate' => $v['SlabRate'],
                                    'charge_type'=>$v['ChargeType'],
                                    'charge_method'=>$v['ChargeMethod']
                                );
...
public function ManageServiceData(){
  ...
  public function Bank_fr_dffrnt_role(){
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($user_info) {
                ...
                if ($user_info['is_block'] == 0) {
                    ...
                    $all_child = $this->Main_model->get_all_usrs();
    ...
    function Update_Event_Pic() {
        if (($_SERVER['REQUEST_METHOD'] == 'POST')) {
            $data = array();
            $e = $this->session->userdata('userid');
            $user_info = $this->Main_model->user_acntid($e);
            
            $validupto = $this->input->post('validuptoevent');
            if ($user_info) {
                if($user_info['is_block']==0 && $user_info['role_id']==1){
                    if (isset($_FILES['File'])) {
                        ...
                        if (!$this->upload->do_upload('File')) {
                            ...
                            $this->Main_model->InsertEvent($userfile, $validupto);
                            
                            $data['error'] = 0;
                            $data['error_desc'] = null;
                            $data['msg'] = 'Your Event Picture is successfully added.';
                        }
                    }

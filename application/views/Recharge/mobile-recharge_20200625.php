...
<script type="text/javascript">
    ...
    $('input[type="tel"]').on('keypress keyup blur', function (e) {
        ...
        if (id == 'mobileno')
        {
			...
			{
                var stri=str.substring(0,4);
				$.post('Recharge/getopr_bymob',{msisdn:stri},function(response){
    ...
    var Rechrge = function () {
        ...
        $('#operator').change(function (e) {
			...
			if(!isMobile){
			  if( opr_sp != ''){
				  console.log(opr_sp)
				 //$('#op_img').html('<img class="center-block" src="assets/operators/'+opr_sp+'.png" />');
				 ...
				 console.log(opr_sp)
			  if(opr_sp != ''){
				// $('#mobile_op').html('<img src="assets/operators/'+opr_sp+'.png" style="width:50px;height:40px;"/>');
				 $('.operator-logo').html('<img width="40" src="assets/operators/' + opr_sp + '.png"  />');
			  }else{
				 //$('#mobile_op').html('');
				  $('.operator-logo').html('');

...
.recharge-detail-list-txt span {
    ...
    text-align: left!important;
        ...
        <div class="col-lg-9">
                           ...
                           <!--  <div class="width-100 payment-status-col" id="payment-status">
                                <div class="width-100 payment-status-header text-center">
                                    <div class="status-icon success-bg text-center"><img src="images/check.svg" width="40"></div>
                                    <div class="font18 fontbold">Payment Successful</div>
                                    <div class="font-medium font16 dark-txt">Payment Receipt</div>
                                </div>

                                <div class="payment-status-innercontent">
                                    <ul class="recharge-detail-list">
                                        <li class="recharge-detail-list-txt"><span class="font-medium light-txt font16 mr-5">Operator Ref</span><span class="fontbold dark-txt font16">9716822835</span></li>
                                        <li class="recharge-detail-list-txt"><span class="font-medium light-txt font16 mr-5">Service Name</span><span class="fontbold dark-txt font16">DTH RECHARGE</span></li>
                                        <li class="recharge-detail-list-txt"><span class="font-medium light-txt font16 mr-5">Customer ID</span><span class="fontbold dark-txt font16">cust230434</span></li>
                                        <li class="recharge-detail-list-txt"><span class="font-medium light-txt font16 mr-5">Transaction Date</span><span class="fontbold dark-txt font16">9716822835</span></li>
                                        <li class="recharge-detail-list-txt"><span class="font-medium light-txt font16 mr-5">Service Type</span><span class="fontbold dark-txt font16">DTH RECHARGE</span></li>
                                        <li class="recharge-detail-list-txt"><span class="font-medium light-txt font16 mr-5">Amount</span><span class="fontbold dark-txt font16">200</span></li>
                                        <li class="sm-pl-10 sm-pr-10">
                                            <div class="font18 light-txt mb-2">THANK YOU FOR YOUR TRANSACTION WITH US</div>
                                            <button class="btn blue-btn back-btn">Pay Another Bill</button>
                                        </li>
                                    </ul>
                                </div>
                            </div>   -->
                           ...
                           <div class="width-100 section-top-subheading mb-3"><h6 class="dark-txt fontbold float-left">DTH RECHARGE</h6> <div class="operator-logo"></div>
                           </div>   
                                <form class="recharge-form-outer">
                                    ...
                                    <div class="form-group form-group-divder-set row ml-0 mr-0">
                                        <label class="col-lg-5 font14 dark-txt font-medium recharge-lbl">CONNECTION NUMBER</label>
                                        <div class="col-lg-7 pl-0 pr-0">
                                            <input type="tel" name="" class="form-control no-brd dark-txt"  placeholder="Enter Connection Number" id="mobileno">
...
<script type="text/javascript">
        ...
        if (id == 'mobileno')
        {
            if (e.type == 'keypress')
            {
                if (k != 8 && k != 9)
                {
                    ...
                    var mb_regex = vRule.allowed;
                    if (!mb_regex.test(k))
                    {
                        return !1
                    }
                    var sw_regex = vRule.startWith;
                    if (length == 0 && !sw_regex.test(k))
                    {
                        return !1
                    }

                    if (length == vRule.maxlen)
                    {
                        return !1
                    }
            ...
            else if (e.type == 'blur')
            {
                 ...
                 var _mobile = vRule.allowed;
                if (!_mobile.test(str))
                {
                    helpBlck({'id': id, 'msg': 'Invalid Connection Number', 'type': 'error'});
                } else {
    ...
    var Rechrge = function () {
        ...
        $.post('Recharge/get_rchrg_srvc_prvdr', {type: 'DTH'}, function (response) {
            if (response) {
                if (response.error == 2)
                {
        ...
        $('#operator').change(function (e) {   
            ...
            if (value === '') {
                ...
                if (value in opr) {
                    ...
                    $('.operator-logo').html('<img width="50" src="assets/operators/' + opr_sp + '.png"  />');
    ...
    var submitRequest = function (la)
    {
        ...
        if (!validate({'id': 'mobileno', 'type': 'DTH', 'data': params.account, 'error': true})) {
            params.valid = false;
        }
        ...
        if (params.valid) {
            ...
            var str = '<div class="confirm-screen-col">';
             str += '<div class="confirm-screen-header text-center font18 fontbold">VERIFY YOUR PAYMENT BILL</div>';
            ...
            $('#tr-confirm').click(function (e) {
                ...
                if (cnfrm === false) {
                    ...
                    $.post('Recharge/dth_recharge', {'data': params}, function (response) {
                                        if (response) {
        ...
        else if (p.type == "DTH")
        {
            ...
            if (_mobile.test(p.data))
            {
                if (p.error == true && (p.data.length > vRule.maxlen))
                {
                     ...
                     helpBlck({'id': p.id, 'msg': 'Invalid Connection Number maximum length should be ' + vRule.maxlen, 'type': 'error'}); 


                }else if ((p.error == true && p.data.length < vRule.minlen)) {


                    helpBlck({'id': p.id, 'msg': 'Invalid Connection Number maximum length should be ' + vRule.minlen, 'type': 'error'});
            ...
            else
            {
                if (p.error == true)
                {
                    ...
                    helpBlck({'id': p.id, 'msg': 'Invalid Connection Number', 'type': 'error'});
                }
